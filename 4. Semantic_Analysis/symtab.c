...
int type_check(int type1, int type2)
{
    ...
    (type1==CHAR_TYPE && type2==CHAR_TYPE) |
     (type1==REAL_TYPE && type2==REAL_TYPE)|
     (type1==REAL_TYPE && type2==INT_TYPE)|
     (type1==SINGLE_TYPE && type2==INT_TYPE)|
     (type1==INT_TYPE && type2==SINGLE_TYPE)|
     (type1==SINGLE_TYPE && type2==SINGLE_TYPE)
     )
    {
        return (type1);
    }
    ...
    else if (type1==REAL_TYPE && type2==INT_TYPE)
    {
        printf("Types are incompatiable\n");
        return(-1);
    }
    else if ((type1==INT_TYPE && type2==CHAR_TYPE) | (type1==CHAR_TYPE && type2==INT_TYPE))
    {
        printf("Type INT and Type CHAR are incompatiable\n");
        return (-1);    
    }
    else if ((type1==CHAR_TYPE && type2==REAL_TYPE)|(type1==REAL_TYPE && type2==CHAR_TYPE))
    {
